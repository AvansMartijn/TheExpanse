...
void pulseTarget() {
	//TODO:: Make this error proof and dynamic and move to helper
	if (targetGrowing) {
		target = helper.scaleOnLocation(target, 1.01, 1.01, 1.01);
		targetSize++;
		if (targetSize >= 50) {
			targetGrowing = false;
		}
		...
		target = helper.scaleOnLocation(target, 0.99, 0.99, 0.99);
		targetSize--;
		if (targetSize <= 0) {
			targetGrowing = true;
		}
...
int main()
{
	target = helper.translateMatrix(threedobj, 0, 0, -50);
